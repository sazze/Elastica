...
class Elastica_Filter_PrefixTest extends PHPUnit_Framework_TestCase
{
	...
	public function testToArray() {
		...
		$filter = new elastica\filter\Prefix($field, $prefix);
	...
	public function testDifferentPrefixes() {
		...
		$query = new elastica\filter\Prefix('name', 'Ba');
		...
		$query = new elastica\filter\Prefix('name', 'Baden');
		$resultSet = $index->search($query);
		$this->assertEquals(2, $resultSet->count());

		$query = new elastica\filter\Prefix('name', 'Baden B');
		$resultSet = $index->search($query);
		$this->assertEquals(1, $resultSet->count());


		$query = new elastica\filter\Prefix('name', 'Baden Bas');
		$resultSet = $index->search($query);
		$this->assertEquals(0, $resultSet->count());
	}
