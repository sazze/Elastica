...
class Elastica_ClientTest extends Elastica_Test
{
	...
	public function testConstruct() {
		$host = 'ruflin.com';
		$port = 9300;
		$client = new elastica\Client(array('host' => $host, 'port' => $port));
	...
	public function testDefaults() {
		...
		$this->assertEquals(elastica\Client::DEFAULT_HOST, 'localhost');
		$this->assertEquals(elastica\Client::DEFAULT_PORT, 9200);
		$this->assertEquals(elastica\Client::DEFAULT_TRANSPORT, 'Http');

		$this->assertEquals(elastica\Client::DEFAULT_HOST, $client->getHost());
		$this->assertEquals(elastica\Client::DEFAULT_PORT, $client->getPort());
		$this->assertEquals(elastica\Client::DEFAULT_TRANSPORT, $client->getTransport());
	}

	public function testServersArray() {
		// Creates a new index 'xodoa' and a type 'user' inside this index
		$client = new elastica\Client(array('servers' => array(array('host' => 'localhost', 'port' => 9200))));
	...
	public function testTwoServersSame() {
		// Creates a new index 'xodoa' and a type 'user' inside this index
		$client = new elastica\Client(array('servers' => array(
	...
	*/
	public function testDeleteIdsIdxStringTypeString() {
		...
		// Adds 1 document to the index
		$doc = new elastica\Document(null, $data);
	...
	*/
	public function testDeleteIdsIdxStringTypeObject() {
		...
		$idxString = $index->getName();
		$this->assertEquals(true, is_string($idxString));
		$this->assertEquals(true, ($type instanceof elastica\Type));
	...
	*/
	public function testDeleteIdsIdxObjectTypeString() {
		...
		// deleteIds are the type we are testing for
		$typeString = $type->getName();
		$this->assertEquals(true, ($index instanceof elastica\Index));
		$this->assertEquals(true, is_string($typeString));
