<?php
/**
 * Abstract_ helper class to implement search indices based on models.
 *
...
abstract class Abstract_ implements \elastica\Searchable
{
	 ...
	 */
	public function __construct(\elastica\Client $client = null) {
		if (!$client) {
			$client = new \elastica\Client();
		}

		if (empty($this->_indexName)) {
			throw new \elastica\exception\Invalid('Index name has to be set');
		}

		if (empty($this->_typeName)) {
			throw new \elastica\exception\Invalid('Type name has to be set');
		}

		$this->_client = $client;
		$this->_index = new \elastica\Index($this->_client, $this->_indexName);
		$this->_type = new \elastica\Type($this->_index, $this->_typeName);
	}
