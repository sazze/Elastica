...
class Client
{
	 ...
	 * @throws \elastica\exception\Invalid If $header or $headerValue is not a string
	 */
	public function addHeader($header, $headerValue) {
		if (is_string($header) && is_string($headerValue)) {
			$this->_config['headers'][$header] = $headerValue;
		} else {
			throw new exception\Invalid('Header must be a string');
		}
	}
	 ...
	 * @throws \elastica\exception\Invalid IF $header is not a string
	 */
	 ...
	 * @throws \elastica\exception\Invalid If docs is empty
	 * @link http://www.elasticsearch.com/docs/elasticsearch/rest_api/bulk/
	 */
	public function addDocuments(array $docs) {

		if (empty($docs)) {
			throw new exception\Invalid('Array has to consist of at least one element');
		}
		$params = array();
	 ...
	 * @throws \elastica\exception\NotImplemented
	 */
	public function deleteDocuments(array $docs) {
		// TODO: similar to delete ids but with type and index inside files
		throw new exception\NotImplemented('not implemented yet');
	}
	 ...
	 * @return elastica\Response Response object
	 * @throws \elastica\exception\Invalid If ids is empty
	 * @link http://www.elasticsearch.com/docs/elasticsearch/rest_api/bulk/
	 */
	public function deleteIds(array $ids, $index, $type) {
		if (empty($ids)) {
			throw new exception\Invalid('Array has to consist of at least one id');
		}
	...
	public function bulk(array $params) {
		if (empty($params)) {
			throw new exception\Invalid('Array has to consist of at least one param');
		}
		...
		if (isset($data['items'])) {
			foreach($data['items'] as $item) {
				...
				if(isset($params['error'])) {
					throw new exception\BulkResponse($response);
				}
			}
		}
