...
class Response {
	...
	public function getEngineTime() {
		...
		if (!isset($data['took'])) {
			throw new exception\NotFound("Unable to find the field [took]from the response");
		}
	...
	public function getShardsStatistics() {
		...
		if (!isset($data['_shards'])) {
			throw new exception\NotFound("Unable to find the field [_shards] from the response");
		}
